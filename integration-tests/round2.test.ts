...
const analysis: any = async(s:any,source:any,_r:any,target:string):Promise<Proposal>=>({id:'fixture-live-proposal',personId:target,candidatePersonIds:[target],text:'A family contributor recalls repairing clocks.',predicate:'craft',evidenceType:'family_recollection',sourceIds:[source.id],spans:[{sourceId:source.id,locator:source.originalLocator,quote:source.originalText}],question:'Does this recollection belong to this relative?',uncertainty:'A recollection, not an independently verified record.',status:'pending',model:'test-stub',origin:'live',createdAt:new Date().toISOString()});
