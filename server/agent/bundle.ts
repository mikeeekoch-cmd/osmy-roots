...
export async function buildRuntimeBundle({
  ...
  const raw = await buildRaw({
    ...
    options: {
      ...
      title: snapshot.sources.some((s) => s.kind === "synthetic_fixture")
        ? "Osmy Roots: A fictional family example"
        : "Osmy Roots: The Family Book",
    },
  });
  ...
  if (snapshot.research) {
    const stage = await (
      await import("./round2")
    ).readStage(snapshot.projectId);
    entries.set(
      "research-stage.json",
      Buffer.from(
        JSON.stringify(
          { graph: stage.graph, manifest: stage.manifest },
          null,
          2,
        ),
      ),
