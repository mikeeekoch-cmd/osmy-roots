...
export function validateSnapshot(raw: unknown): ProjectSnapshot {
  ...
  if(s.research){
    ...
    if(r.bookPlan)for(const chapter of r.bookPlan.chapters)validateSpans(chapter.support,s);
  }
  return s;
}
