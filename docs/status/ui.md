...
- Branch **codex/ui**, tested code **8fea86c**; incorporates engineering **ea320f8**, including **d10eb59, 7acf6eb and e36f6ea**, without discarded commits. Re-fetched at 18:56 UTC; no newer engineering commits were published.
...
- Saved arrival receipts now link to the changed records' actual source evidence. Question heading focus, comparison retry/focus isolation and post-download refresh failures are handled. A successfully delivered run stays closed locally if its status refresh fails; editing a legacy project clears its previous download-complete state. No visible session timer or countdown is rendered.
...
- **Main-agent callout:** wire answerSetupQuestion/prepareFamilyBook/cancelRun and real run creation/pumping in the browser adapter/routes, publish pair/alignment metadata, and deliver the Product fictional twin. Consume this branch before any overlapping UI rewrite. Backend/contracts/dependencies remain lead-owned. The optional interface declaration alone does not activate the round-2 production flow.
- Lead integration detail: the UI honors persisted sealing. Reopening an exported run into a new editable project must reset run/seal state in the server adapter; the older roots-v1 reopen without run metadata already passed here.
