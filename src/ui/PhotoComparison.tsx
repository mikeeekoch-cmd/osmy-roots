...
export function PhotoComparison({ pair, originalUrl, enhancedUrl }: {
  pair: PhotoPair; originalUrl: string; enhancedUrl: string;
}) {
  ...
  const aligned = pair.alignment.mode === "aligned" && usableCrop(pair.alignment.originalCrop) && usableCrop(pair.alignment.enhancedCrop) && (!original || !enhanced || Math.abs(ratio - enhancedRatio) / ratio < 0.03);
  ...
  return <section className="photo-comparison" aria-label="Compare original and enhanced photograph">
    ...
    <p className="comparison-caption">{pair.caption}</p>
    {!aligned && !enhancedFailed && <small>The versions use different framing, so they are shown side by side.</small>}
    <details className="comparison-provenance"><summary>About this enhancement</summary><p>{pair.origin === "prepared" ? "Prepared before this session." : "Created during this session."} {pair.method}</p><p>The original remains the historical evidence. Enhanced details are a visual interpretation.</p></details>
  </section>;
}
