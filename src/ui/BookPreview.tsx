...
export function BookPreview({ snapshot, busy, downloaded, onPrepare, onDownload, onSource, previewUrl }: {
  ...
  return <section className={`current-book-preview ${ready ? "book-ready" : ""}`} aria-label="Current family book">
    ...
    <div className="book-preview-body">
      ...
      {snapshot.bookPassages.length > 0 && <details className="preview-passages"><summary>{ready ? "Read the current book passages" : "Read saved passages, awaiting an update"}</summary>{snapshot.bookPassages.map((p) => <article key={p.id}><p>{p.text}</p>{p.sourceLocators.map((span, i) => <button className="text-button" key={i} onClick={() => onSource(span.sourceId)}>{span.locator}</button>)}</article>)}</details>}
