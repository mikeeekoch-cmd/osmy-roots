...
export function BookPreview({ snapshot, busy, downloaded, onPrepare, onSource, previewUrl, requestError }: {
  snapshot: ProjectSnapshot; busy: boolean; downloaded: boolean; onPrepare?: () => void; onDownload?: () => void; onSource: (id: string) => void; previewUrl?: string; requestError?: string | null;
}) {
  ...
  return <div className="compact-book">
    ...
    {open && <Modal label="Current family book" className="book-details-dialog" onClose={() => { setOpen(false); setPreview(false); }}>
      <section className="book-details-content">
        ...
        <small>Saved project v{snapshot.version}{(edition?.stateVersion ?? run?.book.stateVersion) !== undefined ? ` · Book v${edition?.stateVersion ?? run?.book.stateVersion}` : ""}</small>
        {requested && requestError && <p className="error" role="alert">{requestError}</p>}
        {run?.book.error && <p className="error" role="alert">{run.book.error}</p>}
        <div className="book-preview-actions">{ready && previewUrl && <button onClick={() => setPreview(true)}>Preview current PDF</button>}{!ready && !preparing && onPrepare && <button disabled={busy} onClick={() => {setRequested(true); onPrepare();}}>{failed || requested && requestError ? "Retry book preparation" : "Prepare current edition"}</button>}</div>
